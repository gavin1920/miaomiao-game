...
(function (global) {
  ...
  /* ---------------- 球员运行时 ---------------- */
  /* startStamina：赛季模式的体能延续（cfg.fresh），未提供的球员满体能出场 */
  function mkRt(p, slot, isHome, startStamina) {
    var fit = TAC.slotFit(p, slot, slot.role);
    return {
      p: p, uid: p.__uid, slot: slot, fit: fit,
      on: true, stamina: startStamina != null ? clamp(startStamina, 20, 100) : 100, rating: 6.5,
  ...
  function createMatch(cfg) {
    ...
    var state = {
      ...
      homeAdvSide: cfg.homeAdvSide === 1 ? 1 : 0, /* 主场优势归属侧（v0.3 客场赛制）：默认 0 = 原行为 */
      minute: 0, half: 1, stoppage: 0, stoppage1: 0, finished: false,
      score: [0, 0],
      teams: [], events: [], frames: [], injuries: [],
    ...
    [cfg.home, cfg.away].forEach(function (ts, side) {
      ...
      var rts = tactic.slots.map(function (sl) {
        ...
        if (!p) return null;
        return mkRt(p, { x: sl.x, y: sl.y, zone: sl.zone, code: sl.code, role: sl.role, duty: sl.duty }, side === 0, fresh[p.__uid]);
      });
      ...
      state.teams.push({
        ...
        rt: rts.concat(bench), benchStart: rts.length, subsUsed: 0, subsWindows: 3, ai: !!ts.ai,
        chaseMode: 0
      });
    });
    return state;
  ...
  function resolveShot(state, side, type, shooter, cB, qualityMul, note) {
    ...
    var goalP;
    var ev = { min: state.minute, team: side, type: '', shotType: type, xg: xg, who: shooter.p.name, text: '' };
    if (!isOn) {
      st.xg += 0; /* xG 已计，未射正 */
      ev.type = 'miss';
  ...
  function stepMinute(state) {
    ...
    /* 中场争夺（共识 A7）：修复 gegen 同分钟 possession 双计；
       ...
       主场优势按 homeAdvSide 归属（v0.3：玩家有客场） */
    var homeAdv = state.homeAdvSide === 0 ? 2.2 : -2.2;
    ...
    if (m >= 25 && r < 0.003) { /* 恶性犯规直接红牌：开场阶段裁判更宽容（R3-10） */
      ...
      fouler.rating -= 1.5;
      state.events.push({ min: m, type: 'red', team: side, who: fouler.p.name, text: '红牌！' + fouler.p.name + ' 作为最后一名防守球员拉倒对手，主裁判直接出示红牌！' + team.name + ' 十人应战！' });
    } else if (r < cardP) {
      if (fouler.stats.yellow >= 1) {
        if (state.rng() < 0.33) { /* 两黄变红的二次转化折减，压红牌率 */
          fouler.stats.red = 1; fouler.on = false; st.red++;
          fouler.rating -= 1.2;
          state.events.push({ min: m, type: 'red', team: side, who: fouler.p.name, text: '红牌！' + fouler.p.name + ' 两黄变一红被罚下！' + team.name + ' 十人应战！' });
        } else {
    ...
    /* 伤病（v0.3）：体能越低、对手抢断越凶、末段搏命时风险越高；场均两队合计约 0.3 次 */
    [0, 1].forEach(function (side) {
      var team = state.teams[side], c = side === 0 ? cA : cB;
      var avgStam = c.avgStam || 100;
      var pInj = 0.0018 * (avgStam < 62 ? 1.9 : avgStam < 75 ? 1.25 : 1)
        * (state.teams[1 - side].tactic.instr.tackling === 2 ? 1.3 : 1)
        * (team.chaseMode === 2 ? 1.2 : 1);
      if (state.rng() < pInj) injurePlayer(state, side, c);
    });
  ...
  function attackPhase(state, side, cAtt, cDef) {
    ...
    var homeF = side === state.homeAdvSide ? 1.18 : 0.9;
  ...
  /* ---------------- 伤病 ---------------- */
  /* 伤退必须离场：有名额且窗口未用尽 → 自动换上最契合替补（占用一次窗口）；
     否则少一人作战（menDef/menChance 减员通道自然生效）。
     weeks：赛季模式缺阵轮数（1-3），热身赛仅影响当场 */
  function injurePlayer(state, side, c) {
    var cands = c.on.filter(function (r) { return r.slot.zone !== 'GK'; });
    var victim = weightedPlayer(state, cands.length ? cands : c.on, function (r) {
      return 1 + Math.max(0, 78 - r.stamina) / 12 + (r.slot.duty === 'A' ? 0.4 : 0);
    });
    if (!victim) return;
    var team = state.teams[side];
    var weeks = 1 + Math.floor(state.rng() * 3);
    victim.injured = true;
    victim.injWeeks = weeks;
    state.injuries.push({ side: side, uid: victim.uid, name: victim.p.name, weeks: weeks });
    var rep = (team.subsUsed < 5 && team.subsWindows > 0) ? pickBench(state, side, victim.slot) : null;
    if (rep && doSub(state, side, victim, rep)) {
      state.events.push({ min: state.minute, type: 'injury', team: side, who: victim.p.name, text: '伤情突发！' + victim.p.name + ' 受伤倒地无法坚持，被队医搀扶离场（预计缺阵 ' + weeks + ' 轮），' + rep.p.name + ' 临危受命替补登场。' });
    } else {
      victim.on = false;
      var why = team.subsUsed >= 5 ? '换人名额已用完' : '换人窗口已用尽';
      state.events.push({ min: state.minute, type: 'injury', team: side, who: victim.p.name, text: '伤情突发！' + victim.p.name + ' 无法坚持被抬离球场（预计缺阵 ' + weeks + ' 轮），' + team.name + ' ' + why + '，只能十人应战！' });
    }
  }
  ...
  /* 换人窗口（v0.3 真实规则）：5 名额 + 3 窗口；半场休息的换人不占窗口。
     HT 由引擎状态自判（half=2 且 minute=45 即中场休息点），调用方无需传参 */
  function isHTBreak(state) { return state.half === 2 && state.minute === 45; }
  function doSub(state, side, outRt, inRt) {
    var team = state.teams[side];
    if (team.subsUsed >= 5) return false;
    if (!isHTBreak(state) && team.subsWindows <= 0) return false;
    ...
    team.subsUsed++;
    if (!isHTBreak(state)) team.subsWindows--;
  ...
  function substitute(state, side, outUid, inUid) {
    ...
    if (!isHTBreak(state) && team.subsWindows <= 0) return { ok: false, msg: '换人窗口已用完（3 个）——半场休息时的换人不占窗口' };
